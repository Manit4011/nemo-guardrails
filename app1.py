...
def call_llm_sonet(client, prompt : str, model: str ="anthropic.claude-3-5-sonnet-20240620-v1:0", max_tokens=1024, sys_prompt="You are a helpful AI assistant which returns in JSON.", max_retries=3, retry_delay=8,temperature=0,) -> str:
...
def update_history_route():
    try:
        update_conversation_history()  # Assuming this function is defined in your code
...
def process_query():
    ...
    try:
        ...
        if past_queries:
            ...
            rewrite_response = call_llm_sonet(client, rewrite_prompt)
            if rewrite_response:
                try:
