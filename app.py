...
# # Custom provider wrapper to force ChatBedrock
# def _get_bedrock_chat_model(**kwargs):
#     # Filter out parameters that ChatBedrock doesn't accept
#     if "models" in kwargs: del kwargs["models"]
#     if "engine" in kwargs: del kwargs["engine"]
...
# # Register it as a custom engine
# register_llm_provider("amazon_bedrock_chat_custom", _get_bedrock_chat_model)

# --- Configuration Imports ---
# 1. Load local environment variables (for API_KEY)
load_dotenv()

# 2. Import infrastructure secrets from your AWS Secrets Manager script
# This preserves your existing logic for fetching these IDs dynamically.
...
# --- MANUALLY INITIALIZE LLM ---
# This bypasses the config.yml parsing issues entirely.
...
# Load API_KEY from .env (as requested)
...
def generate_urls(policies):
    ...
    for policy in policies:
        if(policy!="FAQ"):
            temp = policy+".pdf"
            # Ensure S3_BUCKET is set in .env
            bucket_name = os.getenv("S3_BUCKET")
            if bucket_name:
                url = generate_presigned_pdf_url(bucket_name, temp)
...
async def process_query_helper(conv_id, query):
    ...
    # NEW line (Matches your updated disallowed.co):
    refusal_message = "I am an AI assistant for Gemini Solutions HR policies. I cannot answer questions regarding sensitive, personal, illegal, or off-topic matters."
    ...
    client = boto3.client(service_name='bedrock-runtime', region_name=os.getenv("REGION_NAME_1", "us-west-2"), config=config)
    ...
    embed_client = boto3.client(service_name='bedrock-runtime', region_name=os.getenv("REGION_NAME_1", "us-west-2"), config=config)
...
def summary_helper(conv_id, query):
    ...
    if any(word in query.lower() for word in ["summary", "summarize", "summarise"]):
        ...
        # Safe integer conversion to prevent crash
        if len(query_arr) > 1 and query_arr[1].isdigit():
            k = int(query_arr[1])
...
async def process_query():
    ...
    try:
        ...
        # --- ITSM Logic (Commented Out for Future Use) ---
        ...
        #     }
        #     try:
        #         # Using FLASK_API_IP imported from secrets_manager
        #         url = f"http://{FLASK_API_IP}:5000/process_query"
        #         api_response  = call_backend_api(url, headers, payload)
        #         print(api_response)
        #         return api_response
        #     except Exception as e:
...
#         #     }
#         #     try:
#         #         flask_host = os.getenv("FLASK_API_IP")
#         #         url = f"http://{flask_host}:5000/process_query"
#         #         api_response  = call_backend_api(url, headers, payload)
#         #         print(api_response)
#         #         return api_response
#         #     except Exception as e:
